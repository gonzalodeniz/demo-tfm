...
2. Asegura que Argo CD esté instalado en EKS y tenga acceso al repo `demo-tfm` (ajusta `repoURL` en `applicationset.yaml` si usas otra URL accesible desde el clúster).
3. Edita `alumnos.yaml` con los alumnos/apps que necesites.
4. Aplica el ApplicationSet en EKS: `kubectl apply -f applicationset.yaml -n argocd` (o `make sincro` si tienes las credenciales de Gitea/Checkmk configuradas).
